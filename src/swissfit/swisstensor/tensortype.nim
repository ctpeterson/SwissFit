...
proc newTensor*[V:static[int]](s: array[V,int]; x: float32 | float64): SwissTensor[V,type(x)] =
  result.storage := new(new(result,s),type(x))
  for idx in 0..<result.storage.len: result.storage[idx] = x

proc newTensor*[V:static[int],T](s: array[V,int]; x: SwissArray[T]): SwissTensor[V,type(x)] =
